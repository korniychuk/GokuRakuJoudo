...
# base = newest upstream tag (vX.Y.Z, fork tags excluded) reachable from HEAD — never a newer
# upstream release the binary does not contain; N = last -ankor.N for that base + 1.
...
base="$(git tag --list 'v[0-9]*' --merged HEAD --sort=-v:refname | grep -v -- "$FORK_SUFFIX" | head -n 1 || true)"
if [[ -z "$base" ]]; then
