...
(defn parse-special-modi
  ...
  (let [result prevresult
        ...
        result (if (d/consumer-only-k? realkey)
                 (assoc result :consumer_key_code (name realkey))
                 (assoc result :key_code (name realkey)))
...
(defn parse-key
  ...
  (let [{:keys [key ckey pkey any modi mkey]} kinfo
        ...
        result (if (and (not special-modi?) (some? modi))
                 ...
                 result)
        result (if (and (not (:key_code result)) (not (:consumer_key_code result)) (some? key))
                 (let [keycode (parse-keycode key from-to)]
                   (if (d/consumer-only-k? keycode)
                     (assoc result :consumer_key_code (name keycode))
                     (assoc result :key_code (name keycode))))
                 result)
